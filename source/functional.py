...
class PlayMusic():
    def __init__(self) -> None:
        ...
        self.videoCheck = [bool]*1000       # Для внешнего взаимодействия, как и firstPlay, служит для проверки воспроизведения с видео или нет
        self.startSongTime = 0        # Вспомогательная переменная для запоминания времени начала текущей дорожке
        self.currentTime = 0         # Вспомогательная переменная для запоминания времени текущей дорожки
        self.timeAdd = 0            # Вспомогательная переменная для добавления времени
        

    def play(self):
        self.player.set_media(self.Media[self.numberSongName_Play])
        self.player.play()
        self.startSongTime = time.monotonic_ns()
    ...
    def addNext(self, URL, name, video = True):
        ...
        if video:
            ...
            self.videoCheck[self.numberSong] = True
        else:
            best = video_audio.getbestaudio()
            self.videoCheck[self.numberSong] = False
        playurl = best.url
...
if __name__ == "__main__":
    test = PlayMusic()
    #song = test.addNext("https://www.youtube.com/watch?v=0kJdWJXxF3Y", "Reason to Believe", False)
    song = test.addNext("https://www.youtube.com/watch?v=I0WzT0OJ-E0", "Spiritbox - Circle With Me")
    #song = test.addNext("https://www.youtube.com/watch?v=zsCD5XCu6CM", "Somewhere I Belong")

    test.play()
    time.sleep(0.5)
    print(test.player.get_length())
    test.player.set_time(225000)
    while True:
        print(int(test.player.get_time())/1000)
        time.sleep(1)
